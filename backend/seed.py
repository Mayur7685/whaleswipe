...
async def seed_whales():
    ...
    for i, gainer in enumerate(gainers):
        ...
        win_rate = summary.get("counts", {}).get("win_rate", 0) or 0
        # realized_profit_percent is the % gain — what we show on the card
        monthly_pnl = summary.get("pnl", {}).get("realized_profit_percent", 0) or 0
        # Cap extreme values for display
        monthly_pnl = max(min(monthly_pnl, 9999), -999)
